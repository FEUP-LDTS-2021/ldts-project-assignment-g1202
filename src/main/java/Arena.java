    ...
    */
    ...
    private List<Coins> createCoins() {
        ...
        for (int i = 0; i < 2; i++) { //max 2 moedas no ecrã
            Coins newcoin = new Coins(random.nextInt(wall_width -1) , random.nextInt(wall_height - 1) );
            if (!coins.contains(newcoin) && !newcoin.getPosition().equals(player.position.getX()) && !newcoin.getPosition().equals(player.position.getY())) {
                coins.add(newcoin);
            }
